...
templateReferences = ['P100', 'R100', 'R101', 'R102', 'R103']
...
inputBoard = './base-board.kicad_pcb'
#Output file, original file remains unmodified
#outputBoardFile = './boosterilevy/skriptioutput.kicad_pcb'
outputBoardFile = './base-board-arranged.kicad_pcb'

templateRefModulo = 1;	#Difference in the reference numbers between hierarchical sheet
templateRefStart = 100;		#Starting point of numbering in the first hierarchical sheet
move_dx = FromMM(21)		#Spacing between clones in x direction
move_dy = FromMM(90)		#Spacing between clones in y direction
clonesX = 9			#Number of clones in x direction
...
for templateRef in templateReferences:							#For each module in the template schema
    ...
    if templateModule is not None:
        ...
        for i in range(1, numberOfClones):	#0, n-1					#Create list of references to be cloned of this module in the template
#            cloneRefNumber = int(templateReferenceNumber) + (i+1)*templateRefModulo	#Number of the next clone
#            cloneRefNumber = templateReferenceNumber  + (i+1)*templateRefModulo	#Number of the next clone
#            cloneReferences.append(re.sub(templateReferenceNumber, "", templateRef) + str(cloneRefNumber))	#String reference of the next clone
            cloneReferences.append(templateRef + "_" + str(i))	#String reference of the next clone
